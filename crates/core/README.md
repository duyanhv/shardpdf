...
`extractRanges({ input, ranges, signal? })` slices *several* inclusive,
1-based ranges out of one source, parsing it exactly once for any number of
ranges and removing every written slice on error or cancellation. Prefer it
over repeated `extract` calls when you want many slices of one document; the
low-level `Extractor` class exposes the same parse-once handle directly.
...
`validate(path)` inspects a document without a qpdf binary, additionally
verifying that every named destination targets a live page — the corruption
`qpdf --check` accepts silently.

Errors from the native core are `ShardPdfError` with a stable `code`
(`SHARDPDF_PDF_PARSE`, `SHARDPDF_IO`, `SHARDPDF_MALFORMED`,
`SHARDPDF_CONSUMED`, `SHARDPDF_INVALID_ARG`, `SHARDPDF_PANIC`,
`SHARDPDF_DUPLICATE_DESTINATION`, `SHARDPDF_DANGLING_DESTINATION`). Branch on
`error.code`, never on message text. Input mistakes throw plain
`TypeError`/`RangeError`; cancellation surfaces as an `AbortError`.
