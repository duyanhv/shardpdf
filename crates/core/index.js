...
/**
 * @typedef {object} ExtractRangesOptions
 ...
 *
 * @param {ExtractRangesOptions} options
 * @returns {Promise<{sourcePageCount: number, ranges: {output: string, pageCount: number}[]}>}
 */
async function extractRanges(options) {
  if (options === null || typeof options !== "object") {
    throw new TypeError("extractRanges options must be an object");
  }
  ...
  if (!Array.isArray(ranges) || ranges.length === 0) {
    throw new RangeError("extractRanges requires at least one range");
  }
  ...
  signal?.throwIfAborted();
  const extractor = new nativeBinding.Extractor(input);
...
// Public operation facade.
module.exports.merge = merge;
module.exports.extract = extract;
module.exports.getPageCount = getPageCount;
// Raw natives, for hosts that already run in a dedicated child process.
