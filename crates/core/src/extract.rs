...
use crate::assembler::{
    load_document, load_source, push_down_inherited, Assembly, AssemblyError, PdfSource, Result,
    ShardLoadOptions,
};
...
fn extract_selection_from_doc(
    ...
    // fonts, images — and nothing else. Parent/Annots were removed above, so
    // the skip-set only has to stop the walk re-entering the page dicts.
    let selected_set: BTreeSet<ObjectId> = selected.iter().copied().collect();
    let reachable = reachable_from(&source, &selected, &selected_set)?;
    let mut objects: BTreeMap<ObjectId, Object> = BTreeMap::new();
    for id in reachable {
...
mod tests {
    ...
    #[test]
    fn rejects_invalid_ranges_as_malformed() {
        let source = write_source(3, "ranges");
        for (start, end) in [(0, 1), (2, 1), (1, 4), (5, 9)] {
            let result = extract_to(&source, start, end, "ranges");
            assert!(
                // The free-function entry point reports range problems as
                // Malformed; `Extractor::extract_range` uses InvalidSelection.
                matches!(&result, Err(AssemblyError::Malformed(_))),
                "range {start}-{end} must be Malformed"
            );
        }
        std::fs::remove_file(&source).ok();
    }
