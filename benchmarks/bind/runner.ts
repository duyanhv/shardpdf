...
if (
  ...
  console.error(
    "usage: node runner.ts <qpdf|shardpdf|shardpdf-merge> <merge|outline|extract> <manifest> <output>",
  );
  process.exit(2);
}
...
async function runExtract(
  ...
  for (const range of ranges) {
    ...
    if (selectedEngine === "qpdf") {
      ...
      // extractRanges() parses the source once per call; per-range timing
      // here intentionally re-opens per range so qpdf and shardpdf rows stay
      // one-request-per-slice comparable (their production request shape).
      const { extractRanges } = await import("@shardpdf/core");
